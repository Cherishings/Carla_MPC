...
# === Learned A ===
A = np.array([
    [ 0.8727496 ,  0.08776286,  0.00529223],
    [ 0.03464493,  0.6043334 ,  0.08156861],
    [ 0.11895945, -1.0508255 ,  0.94155675]
], dtype=np.float32)

# === Learned B ===
B = np.array([
    [-0.33859003, -0.00324801],
    [ 0.10884423,  0.64811945],
    [-0.2948701 , -0.5543312 ]
], dtype=np.float32)

# === Learned Q ===
Q = np.array([
    [ 0.31762958,  0.38946131, -0.03779831, -0.37012461,  0.22361454],
    [ 0.38946131,  0.48306718,  0.00302240, -0.46399629,  0.17158385],
    [-0.03779831,  0.00302240,  0.48052111,  0.37962192, -0.33238626],
    [-0.37012461, -0.46399629,  0.37962192,  5.9539084/20 ,  7.0638762 ],
    [ 0.22361454,  0.17158385, -0.33238626,  7.0638762 , 33.960064/30 ]
], dtype=np.float32)
...
# def build_x0_from_wp2(wps_ego, cur_speed_mps, tgt_speed_mps):
...
def build_mpc_refs(wps_ego, tgt_speed_mps, horizon=25):
    """
    从预测路点生成参考状态序列 (x_ref)
    格式：shape = (nx, horizon+1)，和 MPC 定义一致
    """
    if wps_ego is None or len(wps_ego) < 2:
        return np.zeros((3, horizon+1), dtype=np.float32)

    # 用 5 个预测点线性插值 horizon+1 个点
    xs = np.linspace(wps_ego[0, 0], wps_ego[-1, 0], horizon+1)
    ys = np.linspace(wps_ego[0, 1], wps_ego[-1, 1], horizon+1)

    e_y   = np.clip(ys, -5.0, 5.0)
    e_psi = np.clip(np.arctan2(ys, np.maximum(1e-3, xs)), -np.pi/2, np.pi/2)
    e_v   = np.clip(tgt_speed_mps * np.ones_like(xs), -10.0, 10.0)

    return np.vstack([e_y, e_psi, e_v])   # shape = (3, horizon+1)
...
def mpc_control(x0, x_refs):
    ...
    for k in range(N):
        constr += [x[:, k + 1] == A @ x[:, k] + B @ u[:, k]]
        x_err = x[:, k] - x_refs[:, k]
        cost += 0.5 * cp.quad_form(cp.hstack([x_err, u[:, k]]), Q)
        # 控制约束
        constr += [u[0, k] <= MAX_THROTTLE, u[0, k] >= 0.0]
        constr += [u[1, k] <= STEER_LIMIT, u[1, k] >= -STEER_LIMIT]
...
def build_x0_from_wp2(wps_ego, cur_speed_mps, tgt_speed_mps):
    """
    构造 x0，使其定义与训练时一致：
    x0 = 当前状态相对于预测轨迹第2个点 (wp2) 的差值
    """
    if wps_ego is None or len(wps_ego) < 2:
        return np.zeros(3, dtype=np.float32)

    # 取 wp2
    x_ref, y_ref = float(wps_ego[1, 0]), float(wps_ego[1, 1])

    # lateral 偏差
    e_y = np.clip(y_ref, -5.0, 5.0)

    # heading 偏差（参考训练里的定义）
    e_psi = np.clip(math.atan2(y_ref, max(1e-3, x_ref)), -np.pi/2, np.pi/2)

    # 速度误差
    e_v = np.clip(tgt_speed_mps - cur_speed_mps, -10.0, 10.0)
    ...
    try:
        while True:
            ...
            if AUTO_DRIVE:
                ...
                x0 = build_x0_from_wp2(wps_ego, cur_speed_mps=speed_mps, tgt_speed_mps=tgt_speed_filtered)
                x_refs = build_mpc_refs(wps_ego, tgt_speed_mps=tgt_speed_filtered, horizon=N)
                # print("e_v (x0[2]) = ", x0[2])
                # print("x_refs[2, :5] = ", x_refs[2, :5])

                try:
                    u0 = mpc_control(x0, x_refs)  # [throttle, steer]
                except Exception as e:
                ...
                print(f"speed={speed_mps:.2f}, tgt={tgt_speed_filtered:.2f}, "
                f"u0={u0}, thr={throttle_cmd:.2f}, brk={brake_cmd:.2f}, brk_prob={brake_prob:.2f}")
